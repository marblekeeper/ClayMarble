...
static void layer_template_HumanHand(CLayerStack* ls) {
    ls->layer_count = 2;
    ls->layers[0].material      = MAT_FLESH;
    ls->layers[0].integrity     = 1;
    ls->layers[0].max_integrity = 1;
    ls->layers[1].material      = MAT_BONE;
    ls->layers[1].integrity     = 1;
    ls->layers[1].max_integrity = 1;
}
...
typedef struct {
    EntityID part_entity[MAX_BODY_PARTS];
} CBodyParts;
...
typedef struct {
    uint32_t   required_anatomy;
    SkillID    required_skill;
    int32_t    min_skill_level;
    BodyPartID body_part_required;
} CapabilityDef;
...
typedef struct {
    ...
    EffectID     on_success;
    int32_t      difficulty;
    int32_t      crit_fail_threshold;
    BodyPartID   crit_fail_bodypart;
    int32_t      crit_fail_damage;
} AffordanceDef;
